...
impl<'a, 'r> PrimitiveBuilder<'a, 'r> {
    ...
    pub fn indices_u32(self, indices: Cow<'a, [u32]>) -> Self {
    ...
    pub fn build(mut self) -> Primitive {
        ...
        for (attribute, tex_coords) in self.tex_coords.iter() {
            let (array_stride, contents, format) = match tex_coords {
                TexCoords::U8(slice) => (2 * 1, cast_slice(slice), wgpu::VertexFormat::Unorm8x2),
                TexCoords::U16(slice) => (2 * 2, cast_slice(slice), wgpu::VertexFormat::Unorm16x2),
                TexCoords::F32(slice) => (2 * 4, cast_slice(slice), wgpu::VertexFormat::Float32x2),
            };
            create_vertex_buffer(
        ...
        for (attribute, colors) in self.colors.iter() {
            let (array_stride, contents, format) = match colors {
                Colors::RgbaU8(slice) => (4 * 1, cast_slice(slice), wgpu::VertexFormat::Unorm8x4),
                Colors::RgbaU16(slice) => (4 * 2, cast_slice(slice), wgpu::VertexFormat::Unorm16x4),
