        ...
        if storm.scenes().len() > 0 {
            ui.horizontal(|ui| {
                ...
                egui::ComboBox::from_id_salt("Scene")
                    .selected_text(storm.scene().map_or("", |scene| &scene.name))
                    .show_ui(ui, |ui| {
                        let mut active_scene = storm.scene().map(|scene| scene.id());
                        ...
                        storm.set_scene(active_scene);
                    })
            });
        }

        if let Some(scene) = storm.scene() {
            ui.separator();
            ui.label("Nodes");
