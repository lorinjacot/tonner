...
pub struct TextureManager {
    ...
    cube_transform_bind_groups: [wgpu::BindGroup; 6],
}
...
impl TextureManager {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        ...
        let create_bind_group = |view| {
            let buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                ...
                contents: cast_slice(&[projection * view]),
        ...
        let cube_transform_bind_groups = [
        ...
        Self {
            ...
            cube_index_buffer,
            cube_vertex_buffer,
            cube_transform_bind_groups,
        }
    }

    pub fn create_texture_2d_bytes(
        &self,
        label: Option<&str>,
        width: u32,
        height: u32,
        ...
        let mip_level_count = match mip {
            TextureMip::Count(count) => count,
            TextureMip::Generate => todo!(),
        };
    ...
    pub fn create_texture_2d_image(
        ...
        let mut create = |bytes: &[u8], format: wgpu::TextureFormat| {
        ...
        match image.color() {
            image::ColorType::Rgb8 => {
                ...
                create(image.to_rgba8().as_bytes(), wgpu::TextureFormat::Rgba8Unorm)
            }
            image::ColorType::Rgba8 => create(image.as_bytes(), wgpu::TextureFormat::Rgba8Unorm),
            image::ColorType::Rgb32F => create(
                image.to_rgba32f().as_bytes(),
                wgpu::TextureFormat::Rgba32Float,
            ),
            _ => todo!("{:?} image format not supported", image.color()),
        }
    }

    pub fn create_texture_2d_pipeline(
        &self,
        label: Option<&str>,
        width: u32,
        height: u32,
        ...
        let mip_level_count = if generate_mips { todo!() } else { 1 };
        ...
        let view = texture.create_view(&wgpu::TextureViewDescriptor::default());
        let texture = Texture2d { texture, view };

        let mut rpass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some(&format!(
                "create_texture_2d_pipeline({}) render pass",
                label.unwrap_or("")
            )),
    ...
    pub fn create_texture_cube_equirectangular(
        &mut self,
        label: Option<&str>,
        width: u32,
        height: u32,
        ...
        let label_prefix = format!(
            "create_texture_cube_equirectangular({})",
        ...
        self.create_texture_cube_pipeline(
            label,
            width,
            height,
            generate_mips,
    ...
    pub fn create_texture_cube_pipeline(
        &self,
        label: Option<&str>,
        width: u32,
        height: u32,
        ...
        for face in 0..6 {
            ...
            render_pass.set_bind_group(0, &self.cube_transform_bind_groups[face], &[]);
        ...
        TextureCube { texture, view }
    }

    fn create_texture_cube_mip_pipeline<A>(
        &mut self,
        label: Option<&str>,
        width: u32,
        height: u32,
        ...
        A: Borrow<wgpu::BindGroup>,
    {
        ...
        for mip in 0..mip_level_count {
            for face in 0..6 {
                ...
                render_pass.set_pipeline(pipeline);
                render_pass.set_bind_group(0, &self.cube_transform_bind_groups[face], &[]);
                render_pass.set_bind_group(1, bind_group(mip).borrow(), &[]);
                render_pass.set_vertex_buffer(0, self.cube_vertex_buffer.slice(..));
                render_pass
                    .set_index_buffer(self.cube_index_buffer.slice(..), wgpu::IndexFormat::Uint16);
                render_pass.draw_indexed(0..CUBE_INDICES.len() as u32, 0, 0..1);
    ...
    pub fn create_texture_cube(
        ...
        let mip_level_count = match mip {
            TextureMip::Count(count) => count,
            TextureMip::Generate => unimplemented!(),
        };

        match source {
            TextureCubeSource::Equirectangular {
                width,
                height,
                format,
                texture,
                ...
                self.create_texture_cube(
                    label,
                    TextureCubeSource::Pipeline {
                        width,
                        height,
                        format,
                        pipeline: &pipeline,
